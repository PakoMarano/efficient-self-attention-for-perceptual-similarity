...
class ViTExtractor(nn.Module):
    ...
    def create_model(model_type: str, load_dir: str = "./models") -> nn.Module:
        """
        Loads the DINO ViT-B/16 model with projection head.
        
        :param model_type: Must be 'dino_vitb16'
        :param load_dir: Location of pretrained ViT checkpoints.
        :return: Tuple of (model, projection_head)
        """
        ...
        sd = torch.load(
            os.path.join(load_dir, 'dino_vitb16_pretrain.pth'), 
            map_location='cpu', 
            weights_only=True
        )
    ...
    def forward(self, x, is_proj=False):
        ...
        :param is_proj: If True, apply projection head
        :return: Model output
        """
        if is_proj:
